...
namespace Printers {
    ...
    bool Atari1020::handleBuffer(QByteArray &buffer, int len)
    {
        ...
        for(int i = 0; i < len; i++) {
            ...
            if (mGraphicsMode) {
                ...
                {
                    mEsc = false;
                } else if (mEsc && b == 7) // CTRL+G: enter graphics mode
                {
                   mGraphicsMode = true;
    ...
    {
        // Since the sample programs send floats, it seems that the
        // Atari 1020 just handles them as ints.
        int result = 0;
        QString number("");
        ...
        bool ok;
        result = floor(number.toFloat(&ok));
        if (ok)
            return result;
        else
